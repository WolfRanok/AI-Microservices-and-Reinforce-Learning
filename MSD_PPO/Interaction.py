...
from Agent_off_policy import *
from ENV import *
from State_update_for_state1 import *
from State_update_for_state2 import *
...
class Environment_Interaction:
    ...
    def __init__(self, ms_image, all_ms):
        ...
        self.each_request_image = self.each_image.copy()  # 服务所需要的微服务实例总和
        self.each_ms_num = self.cal_each_ms_num() # 网络中各个微服务实例的数量
        self.ms_image = np.reshape(ms_image, (USER_NUM * MA_AIMS_NUM)) # 服务上所需要的各类微服务实例数量
        self.sum_ms_aims = int(sum(self.ms_image))   # 待部署的服务总数
        # 初始化服务
        self.ms_aims = all_ms # 微服务实例集
        # self.adv = np.full((sum(ms_image).astype(int),), 0)
        self.T_list = np.full((sum(self.ms_image).astype(int),), 0.0)
        self.T_min_list = np.full((sum(self.ms_image).astype(int),), cal_total_delay(get_deploy(initial_state()),get_each_request_rout(get_deploy(initial_state()))))
        self.dep_ms_count = -1 # 表示尝试部署过的微服务实例数量-1
        self.ms_deploy_idx = get_ms_deploy_order() # 微服务部署顺序，里面保存的当前需要部署的微服务在self.each_ms_num里的索引
    ...
    def cal_each_ms_num(self):
        ...
        for i in range(MA_AIMS_NUM):
            for j in range(USER_NUM):
                each_ms_num[i] += self.ms_image[j][i]
        return each_ms_num
    def option_ms(self):
        """
    ...
    def get_action(self, action_probabilities):
        ...
        dist = Categorical(action_probabilities[0])  # Create a categorical distribution
        action = dist.sample()
    ...
    def get_next_state(self,index, state, action):
        ...
        if self.is_it_sufficient(index, state, action) and sum(self.ms_image) != 0:
            ...
            # 改变服务部署方案与资源剩余情况
            mid_state = updata_state(mid_state, action, index)
        ...
        if sum(self.ms_image) != 0:
            start = MA_AIMS_NUM * NODE_NUM + 3 * 2 * NODE_NUM
            cur_ms_information = mid_state[start:(start + 4)]
            cur_request_information = mid_state[(start + 4):(start + 8)]
            if self.ms_image[self.ms_deploy_idx[self.now_dep_ms_class_num%len(self.ms_deploy_idx)]] == 0:
                ...
                if isinstance(next_ms,MS):
                    ...
                    cur_ms_information[0] = next_ms.id+MS_NUM
            ...
            mid_state[start:(start + 4)] = cur_ms_information
            mid_state[(start + 4):(start + 8)] = cur_request_information
        return mid_state
    def get_next_state_for_new_state(self,index, state, action):
        """
        依照概率选择下一个状态
        :param state: index 微服务编号，AI微服务接着基础微服务后面继续编号
        :param state: state
        ...
        if self.is_it_sufficient(index, state, action) and sum(self.ms_image) != 0:
            ...
            self.ms_image[this_ms_index] -= 1
            # 改变服务部署方案与资源剩余情况
            mid_state = updata_state_for_new_state(mid_state, action, index).copy()
            # start = MA_AIMS_NUM*NODE_NUM+ 3*2*NODE_NUM
            # state[start:start+USER_NUM*MA_AIMS_NUM] = self.ms_image.copy()
        else:
            # 此时，微服务实例部署失败，但是还是需要更新state中的ms_image，但不更新部署方案和资源占用情况
        ...
        if sum(self.ms_image) != 0:
            if self.ms_image[self.ms_deploy_idx[self.now_dep_ms_class_num % len(self.ms_deploy_idx)]] == 0:
                self.now_dep_ms_class_num += 1
            next_ms = all_ms[self.ms_deploy_idx[self.now_dep_ms_class_num % len(self.ms_deploy_idx)] % MA_AIMS_NUM]
            next_user = users[int(self.ms_deploy_idx[self.now_dep_ms_class_num % len(self.ms_deploy_idx)] / MA_AIMS_NUM)]
            next_request = requests.get(next_user)
            current_deploy = get_deploy_for_new_state(mid_state)  # 获得已经更新过的服务部署方案
            current_rout = get_this_request_rout(current_deploy, next_user, next_request)  # 计算当前服务请求的路由
            mid_state = updata_rout_for_new_state(mid_state, current_rout)  # 更新请求路由
            mid_state[8] = cal_this_service_delay(current_deploy, next_user, current_rout)  # 计算请求时延
            if isinstance(next_ms, MS):
                next_ms_id = next_ms.id
            else:
                next_ms_id = next_ms.id + MS_NUM
            # 更新微服务部署类别
            cur_ms_information = mid_state[:5]
            if next_ms_id != cur_ms_information[0]:
                cur_ms_information[0] = next_ms_id
                cur_ms_information[1] = data[next_ms_id]
                cur_ms_information[2] = next_ms.get_cpu()
                cur_ms_information[3] = next_ms.get_gpu()
                cur_ms_information[4] = next_ms.get_memory()
            # 检查用户信息是否需要更新
            cur_request_information = mid_state[5:5 + 4 + MA_AIMS_NUM]
            if next_user.id != cur_request_information[0]:
                # 更新服务信息，只有在当前服务完成部署后才需要进行
                cur_request_information[0] = next_user.id
                cur_request_information[1] = next_user.get_lamda()
                cur_request_information[3] = cal_this_service_delay(current_deploy, next_user,current_rout)  # 计算请求时延
                cur_request_information[4:4 + MA_AIMS_NUM] = get_request_chain(requests.get(next_user))  # 获得服务请求链
            mid_state[:5] = cur_ms_information
            mid_state[5:5 + 4 + MA_AIMS_NUM] = cur_request_information
        else:
            current_deploy = get_deploy_for_new_state(mid_state)  # 获得已经更新过的服务部署方案
            this_user = users[-1]
            this_requeset = requests.get(this_user)
            current_rout = get_this_request_rout(current_deploy, this_user, this_requeset)  # 计算当前服务请求的路由
            mid_state = updata_rout_for_new_state(mid_state, current_rout)  # 更新请求路由
        return mid_state
    ...
    def is_it_sufficient(self, index, state, action):
        ...
        resource = get_resource_for_new_state(state)
        # 需要消耗的资源情况
    ...
    def get_reward(self, flag, ms_index, state=None, next_state=None, episode_count=1):
        ...
        T = self.get_T(state)
        res = get_resource_for_new_state(next_state)
        loadb = cal_load_balance(res)

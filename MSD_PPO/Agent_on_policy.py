...
class PPOAgent:
    def __init__(self, state_dim, action_dim, action_type='binary',  # 修正参数名
                 gamma=0.99, lambad_=0.5, clip_range=0.1, ppo_epochs=50,
                 tau=0.005, value_coeff=0.25):
        # 根据action_type选择网络结构
        if action_type == 'continuous':
            raise NotImplementedError("Continuous action not supported in current setup")
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.actor = network.LSTM_Actor(state_dim, action_dim).to(self.device)
        self.actor_target = network.LSTM_Actor(state_dim, action_dim).to(self.device)
        self.critic = network.LSTM_Critic(state_dim).to(self.device)
        self.critic_target = network.LSTM_Critic(state_dim).to(self.device)
    ...
    def compute_advantage(self, td_delta):
        # td_delta = torch.from_numpy(td_delta.detach().cpu())
        td_delta = td_delta.detach().cpu().numpy()
        advantage_list = []
        advantage = 0.0
        for delta in td_delta[::-1]:
            advantage = self.gamma * self.lambad_ * advantage + delta
            advantage_list.append(advantage)
        advantage_list.reverse()
        return torch.tensor(np.array(advantage_list), dtype=torch.float)

    def update_model(self, transition_dict):
        ...
        advantages = self.compute_advantage(td_delta.to(self.device)).to(self.device)
        ...
        old_log_probs = torch.log(prob.gather(1, actions) + 1e-8).detach()
        ...
        for p in range(self.ppo_epochs):
            ...
            surr2 = torch.clamp(ratio, 1 - self.clip_range, 1 + self.clip_range) * advantages  # 截断
            actor_loss = torch.mean(-torch.min(surr1, surr2))  # PPO损失函数
            critic_loss = torch.mean(F.mse_loss(self.critic(states), td_target.detach()))
            assert torch.isnan(actor_loss).sum() == 0, print( f'actor_loss:{actor_loss}, old_log_probs:{old_log_probs}, new_log_probs:{new_log_probs}， surr1:{surr1}, surr2:{surr2}')
            assert torch.isnan(critic_loss).sum() == 0, print(f'critic_loss:{critic_loss}')
            pollicy_losses[p] = actor_loss
            value_losses[p] = critic_loss
            total_losses[p] = actor_loss + critic_loss
        ...
        print(f"Actor Grad Norm: {actor_grad_norms.mean():.4f}, Critic Grad Norm: {critic_grad_norms.mean():.4f}")

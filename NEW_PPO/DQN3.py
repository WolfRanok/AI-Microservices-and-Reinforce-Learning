...
def save_model(agent, model_path):
    ...
    torch.save(agent.target_net.state_dict(), os.path.join(model_path, "dqn_target_net_length_of_service_4_6.pth"))
    print("模型已保存！")
...
def train(env, args):
    # 创建日志目录
    os.makedirs(os.path.dirname('Environmental_parameters/length_of_service/Training_data/dqn_training_length_of_service_4_6.csv'), exist_ok=True)
    os.makedirs(os.path.dirname('Environmental_parameters/length_of_service/Evaluation_data/dqn_evaluate_length_of_service_4_6.csv'), exist_ok=True)
    ...
    while total_episodes < args.max_train_episodes:
        ...
        if episode_losses:
            ...
            write_training_data('Environmental_parameters/length_of_service/Training_data/dqn_training_length_of_service_4_6.csv', total_episodes, avg_episode_loss)
...
def evaluate_policy(env, agent, args, evaluate_num=None):
    ...
    if evaluate_num is not None:
        write_evaluate_data('Environmental_parameters/length_of_service/Evaluation_data/dqn_evaluate_length_of_service_4_6.csv', evaluate_num, evaluate_reward, evaluate_sum_delay)
...
if __name__ == "__main__":
    ...
    env_args = Environmental_parameters.length_of_service.length_of_service_4_6.get_args()
    env = ENV(env_args)
